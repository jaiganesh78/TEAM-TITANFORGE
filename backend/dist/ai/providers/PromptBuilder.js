...
4. You must format your final output strictly as JSON matching the schema requested. Do not wrap the JSON in Markdown formatting (e.g. do not write \`\`\`json ... \`\`\`), output raw JSON text only.`;
     ...
     */
    static buildContextPrompt(contextPackage) {
        ...
        if (relevantKnowledge && relevantKnowledge.length > 0) {
            relevantKnowledge.forEach((c, index) => {
     ...
     */
    static buildSwotPrompt(contextText) {
        return `${contextText}
...
`;
     ...
     */
    static buildCompetitorPrompt(contextText, competitorData) {
        return `${contextText}
...
`;
     ...
     */
    static buildRecommendationsPrompt(contextText, sectorConfigText, gapAnalysisText) {
        return `${contextText}
