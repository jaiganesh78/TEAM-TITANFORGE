...
const AnalyticsEngineService_1 = require("./AnalyticsEngineService");
...
async function createTestEnvironment() {
    const org = await prisma_1.prisma.organization.create({ data: { name: 'Analytics Engine Test Org' } });
...
async function cleanup(bizId) {
    ...
    if (biz) {
        await prisma_1.prisma.organization.deleteMany({ where: { id: biz.organizationId } });
    }
    await prisma_1.prisma.business.deleteMany({ where: { id: bizId } });
}
...
async function runAnalyticsTests() {
    ...
    try {
        ...
        const parsed = ResponseParser_1.ResponseParser.parseAndValidate(mockRes.text, ResponseParser_1.BusinessHealthScoreSchema);
        ...
        const sessionId = await AnalyticsEngineService_1.AnalyticsEngineService.runAnalyticsEngine(bizId);
        ...
        const snap = await prisma_1.prisma.businessEvolutionSnapshot.findFirst({
        ...
        const assets = await prisma_1.prisma.analyticsAsset.findMany({
            where: { businessId: bizId }
        });
        ...
        const V2_sessionId = await AnalyticsEngineService_1.AnalyticsEngineService.runAnalyticsEngine(bizId);
        const snaps = await prisma_1.prisma.businessEvolutionSnapshot.findMany({
            where: { businessId: bizId },
            orderBy: { version: 'asc' }
        });
        assert(snaps.length >= 2, 'Two snapshot versions generated for comparison');
        const compare = await BusinessEvolutionService_1.BusinessEvolutionService.compareSnapshots(bizId, V2_sessionId, snaps[0].id, snaps[1].id);
        ...
        const dec = await BusinessEvolutionService_1.BusinessEvolutionService.logDecision(bizId, 'Test Decision', 'Reason details', 'Evidence notes', 'analytics-engine', 90.0, 95.0);
        assert(dec !== null, 'Decision audit log registered successfully');
        const listDec = await prisma_1.prisma.decisionIntelligence.findMany({ where: { businessId: bizId } });
        assert(listDec.length > 0, 'Decision items retrievable from database');
        passed++;
        // ─────────────────────────────────────────────
        console.log('\nTest 8: Forecast Accuracy calculations');
        const acc = await BusinessEvolutionService_1.BusinessEvolutionService.calculateForecastAccuracy(bizId);
        ...
        const storyText = await BusinessEvolutionService_1.BusinessEvolutionService.generateQuarterlyStory(bizId);
