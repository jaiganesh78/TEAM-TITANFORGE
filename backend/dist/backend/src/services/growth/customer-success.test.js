...
const CustomerSuccessService_1 = require("./CustomerSuccessService");
...
async function createTestEnvironment() {
    const org = await prisma_1.prisma.organization.create({ data: { name: 'Customer Success Test Org' } });
    ...
    const pipeline = await RevenuePipelineService_1.RevenuePipelineService.initializePipeline(biz.id, 'B2B Enterprise Pipeline');
    ...
    // Create Lead Session
    const leadSession = await prisma_1.prisma.leadSession.create({
        data: {
            businessId: biz.id,
            status: 'COMPLETED'
        }
    ...
    await prisma_1.prisma.leadProfile.create({
        data: {
    ...
    await prisma_1.prisma.customerSuccessSession.create({
        data: {
    ...
    await prisma_1.prisma.customerDigitalTwin.create({
        data: {
            businessId: biz.id,
...
async function runCustomerSuccessTests() {
    ...
    try {
        ...
        const twin = await prisma_1.prisma.customerDigitalTwin.findFirst({ where: { businessId: bizId } });
        assert(twin !== null, 'Initial digital twin retrieved');
        const sessionId = await CustomerSuccessService_1.CustomerSuccessService.runCustomerSuccessEngine(bizId, twin.id);
        ...
        const parseHealth = ResponseParser_1.CustomerHealthSchema.safeParse({
        ...
        const valRealization = await prisma_1.prisma.customerValueRealization.findFirst({
        ...
        const history = await prisma_1.prisma.customerHistory.findMany({
            where: { twinId: twin.id }
        });
        ...
        const asset = await prisma_1.prisma.customerSuccessAsset.findFirst({
            where: { sessionId }
        });
        ...
        const audit = await CustomerSuccessService_1.CustomerSuccessService.handleFeedback(bizId, sessionId, rec.id, 'ACCEPT', 'Deploying telemetry cache rules verified on staging.');
        assert(audit.action === 'ACCEPT', 'Action ACCEPT logged');
        const updatedRec = await prisma_1.prisma.customerRecommendation.findUnique({
            where: { id: rec.id }
        });
        assert(updatedRec.status === 'APPROVED', 'Recommendation status updated to APPROVED');
        const execPlan = await prisma_1.prisma.customerExecutionPlan.findFirst({
        ...
        const bench = await prisma_1.prisma.customerBenchmark.findFirst({
            where: { sessionId }
        });
