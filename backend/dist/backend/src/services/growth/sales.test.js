...
const SalesEngineService_1 = require("./SalesEngineService");
...
async function createTestEnvironment() {
    const org = await prisma_1.prisma.organization.create({ data: { name: 'Sales Engine Test Org' } });
...
async function runSalesTests() {
    ...
    try {
        ...
        const parsed = ResponseParser_1.ResponseParser.parseAndValidate(mockRes.text, ResponseParser_1.OpportunityAnalysisSchema);
        ...
        const sessionId = await SalesEngineService_1.SalesEngineService.runSalesEngine(bizId);
        ...
        const snapshot = await prisma_1.prisma.revenueIntelligenceSnapshot.findFirst({
        ...
        const rAssets = await prisma_1.prisma.revenueAsset.findMany({
            where: { businessId: bizId }
        });
        ...
        const benchmarks = await RevenueIntelligenceService_1.RevenueIntelligenceService.calculateBenchmarks(bizId);
        ...
        const scores = await RevenueIntelligenceService_1.RevenueIntelligenceService.calculateExecutiveScores(bizId);
        ...
        const sessionRecs = await prisma_1.prisma.salesRecommendation.findMany({
            where: { sessionId }
        });
        assert(sessionRecs.length > 0, 'Session recommendations mapped');
        // Handle ACCEPT action
        await SalesEngineService_1.SalesEngineService.handleFeedback(bizId, sessionId, 'ACCEPT');
        const acceptedRec = await prisma_1.prisma.salesRecommendation.findFirst({
            where: { sessionId, status: 'APPROVED' }
        });
        assert(acceptedRec !== null, 'Recommendation APPROVED in database');
        const execPlan = await prisma_1.prisma.salesExecutionPlan.findFirst({
            where: { sessionId }
        });
        assert(execPlan !== null, 'Execution Plan created for accepted strategy');
        const acceptedMemory = await prisma_1.prisma.salesMemory.findFirst({
            where: { businessId: bizId }
        });
