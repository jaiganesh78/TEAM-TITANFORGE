...
class StrategyEngineService {
     ...
     */
    static async generateStrategy(businessId) {
        // 1. Validate readiness before running
        const readiness = await AIReadinessService_1.AIReadinessService.calculateReadiness(businessId, 'strategy-engine');
        ...
        const contextPackage = await BusinessContextService_1.BusinessContextService.assembleContext(businessId, 'Company Growth Strategy');
        // 3. Create context snapshot record
        const snapshot = await BusinessContextService_1.BusinessContextService.createSnapshot(businessId, 'Strategy Session Target', contextPackage.growthTwinSummary?.version || 1);
        ...
        try {
            ...
            const updatedSession = await prisma_1.prisma.strategySession.update({
                where: { id: session.id },
                data: {
                    status: 'COMPLETED',
            ...
            for (const rec of recommendationsList) {
                const recommendation = await prisma_1.prisma.strategyRecommendation.create({
                    data: {
                        sessionId: session.id,
                        businessId,
                        title: rec.title,
                ...
                for (const fact of factsUsed) {
                    await prisma_1.prisma.strategyEvidence.create({
                        data: {
                            sessionId: session.id,
            ...
            await prisma_1.prisma.strategySession.update({
                where: { id: session.id },
                data: { status: 'FAILED' }
            });
            throw err;
     ...
     */
    static async syncToAIOperatingContext(businessId, strategicAssets) {
        if (!strategicAssets)
            return;
        ...
        const newOpportunities = strategicAssets.opportunities?.opportunities?.map((o) => o.title) || [];
     ...
     */
    static async submitFeedback(businessId, recommendationId, action, feedbackText, editedGoals) {
        const rec = await prisma_1.prisma.strategyRecommendation.findUnique({
        ...
        await prisma_1.prisma.strategyFeedback.create({
            data: {
        ...
        const updated = await prisma_1.prisma.strategyRecommendation.update({
            where: { id: recommendationId },
            data: { status: statusMap[action] }
        });
        // If approved, initialize execution plan
        if (action === 'ACCEPT') {
            await prisma_1.prisma.strategyExecutionPlan.create({
                data: {
                    sessionId: rec.sessionId,
                    recommendationId,
                    businessId,
        ...
        await prisma_1.prisma.strategyHistory.create({
            data: {
                sessionId: rec.sessionId,
                recommendationId,
                businessId,
