...
const LeadEngineService_1 = require("./LeadEngineService");
...
async function createTestEnvironment() {
    const org = await prisma_1.prisma.organization.create({ data: { name: 'Lead Engine Test Org' } });
    ...
    await prisma_1.prisma.businessKPI.createMany({
        data: [
    ...
    await prisma_1.prisma.knowledgeChunk.createMany({ data: chunkData });
    // Create a StrategySession to ground Strategic Assets
    await prisma_1.prisma.strategySession.create({
        data: {
    ...
    await prisma_1.prisma.marketingSession.create({
        data: {
            businessId: TEST_BIZ_ID,
            status: 'COMPLETED',
            contextVersion: 1,
...
async function cleanup(bizId) {
    ...
    await prisma_1.prisma.leadHistory.deleteMany({ where: { businessId: bizId } });
    await prisma_1.prisma.leadFeedback.deleteMany({ where: { businessId: bizId } });
    await prisma_1.prisma.leadExecutionPlan.deleteMany({ where: { businessId: bizId } });
    await prisma_1.prisma.leadEvidence.deleteMany({ where: { businessId: bizId } });
    await prisma_1.prisma.leadScore.deleteMany({ where: { businessId: bizId } });
    await prisma_1.prisma.leadProfile.deleteMany({ where: { businessId: bizId } });
    await prisma_1.prisma.leadSource.deleteMany({ where: { businessId: bizId } });
    await prisma_1.prisma.leadJourney.deleteMany({ where: { businessId: bizId } });
    await prisma_1.prisma.leadRecommendation.deleteMany({ where: { businessId: bizId } });
    await prisma_1.prisma.leadForecast.deleteMany({ where: { businessId: bizId } });
    await prisma_1.prisma.leadPlaybook.deleteMany({ where: { businessId: bizId } });
    await prisma_1.prisma.leadMemory.deleteMany({ where: { businessId: bizId } });
    await prisma_1.prisma.leadSession.deleteMany({ where: { businessId: bizId } });
    await prisma_1.prisma.revenuePipelineHistory.deleteMany({ where: { businessId: bizId } });
    await prisma_1.prisma.revenuePipelineStage.deleteMany({ where: { businessId: bizId } });
    await prisma_1.prisma.revenuePipeline.deleteMany({ where: { businessId: bizId } });
    await prisma_1.prisma.strategySession.deleteMany({ where: { businessId: bizId } });
    await prisma_1.prisma.marketingSession.deleteMany({ where: { businessId: bizId } });
    const biz = await prisma_1.prisma.business.findUnique({ where: { id: bizId } });
    if (biz) {
        await prisma_1.prisma.business.delete({ where: { id: bizId } });
...
async function runLeadTests() {
    ...
    try {
        ...
        const pipeline = await RevenuePipelineService_1.RevenuePipelineService.initializePipeline(bizId, 'B2B Enterprise Pipeline');
        ...
        const stages = await prisma_1.prisma.revenuePipelineStage.findMany({ where: { pipelineId: pipeline.id } });
        ...
        const sessionId = await LeadEngineService_1.LeadEngineService.runLeadEngine(bizId);
        ...
        const transitioned = await RevenuePipelineService_1.RevenuePipelineService.transitionLead(bizId, firstLead.id, 'Engaged', 'Test Runner');
        // Verify stage update
        const leadAfterTransition = await prisma_1.prisma.leadProfile.findUnique({
        ...
        const historyLogs = await prisma_1.prisma.revenuePipelineHistory.findMany({ where: { leadId: firstLead.id } });
        ...
        // Health diagnostics
        const health = await RevenuePipelineService_1.RevenuePipelineService.getPipelineHealth(bizId);
        ...
        const feedback = await LeadEngineService_1.LeadEngineService.handleFeedback(bizId, sessionId, 'ACCEPT', 'This outbound sequence meets target CAC goals');
        ...
        const recAfterFeedback = await prisma_1.prisma.leadRecommendation.findFirst({ where: { sessionId } });
        ...
        const execPlan = await prisma_1.prisma.leadExecutionPlan.findFirst({ where: { sessionId } });
        ...
        const opContext = await prisma_1.prisma.aIOperatingContext.findUnique({ where: { businessId: bizId } });
        ...
        const memoryCtx = await LeadMemoryService_1.LeadMemoryService.getMemoryContext(bizId);
        ...
        const readiness = await AIReadinessService_1.AIReadinessService.calculateReadiness(bizId, 'lead-generation-engine');
