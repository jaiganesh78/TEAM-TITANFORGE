...
exports.CustomerSuccessService = void 0;
const prisma_1 = require("../../database/prisma");
const CustomerSuccessWorkflow_1 = require("./CustomerSuccessWorkflow");
const CustomerValueRealizationService_1 = require("./CustomerValueRealizationService");
class CustomerSuccessService {
    /**
     * Runs the complete Customer Success graph for a given customer digital twin.
     */
    static async runCustomerSuccessEngine(businessId, twinId) {
        // 1. Resolve or initialize Customer Digital Twin
        let activeTwin = twinId ? await prisma_1.prisma.customerDigitalTwin.findUnique({ where: { id: twinId } }) : null;
        if (!activeTwin) {
            // Find first existing twin for this business, or create a mock enterprise customer
            activeTwin = await prisma_1.prisma.customerDigitalTwin.findFirst({ where: { businessId } });
            if (!activeTwin) {
                // Resolve lead profile if available to link
                const lead = await prisma_1.prisma.leadProfile.findFirst({ where: { businessId } });
                activeTwin = await prisma_1.prisma.customerDigitalTwin.create({
                    data: {
                        businessId,
        ...
        const session = await prisma_1.prisma.customerSuccessSession.create({
            data: {
                businessId,
                status: 'PENDING',
        ...
        try {
            ...
            const graph = (0, CustomerSuccessWorkflow_1.createMasterCustomerSuccessGraph)();
            const initialState = {
                businessId,
            ...
            await prisma_1.prisma.customerHealth.create({
                data: {
                    sessionId: session.id,
                    businessId,
            ...
            await prisma_1.prisma.customerJourney.create({
                data: {
                    sessionId: session.id,
                    businessId,
                    twinId: activeTwin.id,
            ...
            await prisma_1.prisma.customerLifecycle.create({
                data: {
                    sessionId: session.id,
                    businessId,
                    twinId: activeTwin.id,
            ...
            await prisma_1.prisma.customerSupportCase.create({
                data: {
                    sessionId: session.id,
                    businessId,
                    twinId: activeTwin.id,
            ...
            await prisma_1.prisma.customerSentiment.create({
                data: {
                    sessionId: session.id,
                    businessId,
                    twinId: activeTwin.id,
            ...
            await prisma_1.prisma.renewalForecast.create({
                data: {
                    sessionId: session.id,
                    businessId,
                    twinId: activeTwin.id,
            ...
            await prisma_1.prisma.renewalPlan.create({
                data: {
                    sessionId: session.id,
                    businessId,
                    twinId: activeTwin.id,
            ...
            const expOp = await prisma_1.prisma.expansionOpportunity.create({
                data: {
                    sessionId: session.id,
                    businessId,
                    twinId: activeTwin.id,
            ...
            await prisma_1.prisma.crossSellOpportunity.create({
                data: {
                    sessionId: session.id,
                    businessId,
                    twinId: activeTwin.id,
            ...
            await prisma_1.prisma.upsellOpportunity.create({
                data: {
                    sessionId: session.id,
                    businessId,
                    twinId: activeTwin.id,
            ...
            await prisma_1.prisma.churnRisk.create({
                data: {
                    sessionId: session.id,
                    businessId,
                    twinId: activeTwin.id,
            ...
            await prisma_1.prisma.retentionStrategy.create({
                data: {
                    sessionId: session.id,
                    businessId,
                    twinId: activeTwin.id,
            ...
            await CustomerValueRealizationService_1.CustomerValueRealizationService.evaluateValueRealization(businessId, activeTwin.id, session.id);
            // J. Advocacy predictions
            const advocacy = finalState.customerAdvocacy || {};
            await prisma_1.prisma.customerAdvocacy.create({
                data: {
                    sessionId: session.id,
                    businessId,
                    twinId: activeTwin.id,
            ...
            await prisma_1.prisma.customerSuccessAsset.create({
                data: {
                    sessionId: session.id,
                    businessId,
            ...
            await prisma_1.prisma.customerSuccessScore.create({
                data: {
                    sessionId: session.id,
                    businessId,
                    twinId: activeTwin.id,
            ...
            for (const rec of recommendations) {
                await prisma_1.prisma.customerRecommendation.create({
                    data: {
                        sessionId: session.id,
                        businessId,
                        twinId: activeTwin.id,
            ...
            for (const ev of timeline) {
                await prisma_1.prisma.customerHistory.create({
                    data: {
                        sessionId: session.id,
                        businessId,
                        twinId: activeTwin.id,
            ...
            await prisma_1.prisma.customerBenchmark.create({
                data: {
                    sessionId: session.id,
                    businessId,
            ...
            await prisma_1.prisma.customerSuccessSnapshot.create({
                data: {
                    sessionId: session.id,
                    businessId,
            ...
            await prisma_1.prisma.customerDigitalTwin.update({
            ...
            await prisma_1.prisma.customerSuccessSession.update({
                where: { id: session.id },
                data: { status: 'SUCCESS' }
            });
            // Synchronize playbooks to Customer Memory & Business context
            await prisma_1.prisma.customerMemory.create({
                data: {
                    businessId,
                    twinId: activeTwin.id,
                    acceptedStrategies: '[]',
     ...
     */
    static async handleFeedback(businessId, sessionId, recommendationId, action, feedbackText) {
        const feedback = await prisma_1.prisma.customerFeedbackItem.create({
            data: {
                recommendationId,
                action,
                feedbackText
        ...
        const rec = await prisma_1.prisma.customerRecommendation.update({
            where: { id: recommendationId },
            data: { status: action === 'ACCEPT' ? 'APPROVED' : 'REJECTED' }
        });
        // Create execution task logs if recommendation was approved
        if (action === 'ACCEPT') {
            await prisma_1.prisma.customerExecutionPlan.create({
                data: {
                    sessionId,
                    businessId,
                    recommendationId,
            ...
            const memory = await prisma_1.prisma.customerMemory.findFirst({
                where: { businessId }
            });
            if (memory) {
                const approved = JSON.parse(memory.acceptedStrategies || '[]');
                approved.push(rec.title);
                await prisma_1.prisma.customerMemory.update({
                    where: { id: memory.id },
...
exports.CustomerSuccessService = CustomerSuccessService;
