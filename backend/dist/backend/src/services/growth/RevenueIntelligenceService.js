...
class RevenueIntelligenceService {
     ...
     */
    static async calculateBenchmarks(businessId) {
        const leads = await prisma_1.prisma.leadProfile.findMany({
            where: { businessId },
            include: { stage: true }
        });
        const histories = await prisma_1.prisma.revenuePipelineHistory.findMany({
            where: { businessId }
        });
        // 1. Average Deal Size
        ...
        const leadScores = await prisma_1.prisma.leadScore.findMany({ where: { businessId } });
     ...
     */
    static async calculateExecutiveScores(businessId) {
        ...
        const overallExecutiveRevenueScore = Math.round(pipelineHealth * 0.25 +
     ...
     */
    static async createPerformanceSnapshot(businessId, sessionId) {
        const benchmarks = await this.calculateBenchmarks(businessId);
        ...
        const snapshot = await prisma_1.prisma.revenueIntelligenceSnapshot.create({
            data: {
                businessId,
                sessionId,
     ...
     */
    static async createRevenueAssets(businessId, sessionId) {
        const benchmarks = await this.calculateBenchmarks(businessId);
        const execScores = await this.calculateExecutiveScores(businessId);
        const assets = [
            {
        ...
        for (const asset of assets) {
            const dbAsset = await prisma_1.prisma.revenueAsset.create({
                data: {
                    businessId,
                    sessionId,
...
exports.RevenueIntelligenceService = RevenueIntelligenceService;
