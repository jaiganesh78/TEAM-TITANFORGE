...
const GrowthTwinService_1 = require("./GrowthTwinService");
...
async function createTestBusiness() {
    const org = await prisma_1.prisma.organization.create({ data: { name: 'Growth Test Org' } });
...
async function cleanup(businessId) {
    const biz = await prisma_1.prisma.business.findUnique({ where: { id: businessId }, include: { organization: true } });
    if (biz) {
...
async function runTests() {
    ...
    try {
        ...
        await GrowthTwinService_1.GrowthTwinService.initDomains(businessId);
        ...
        if (summary.coveredDomains === 0)
            throw new Error('No domains covered after sync');
        ...
        await GrowthTwinService_1.GrowthTwinService.syncDomain(businessId, 'MARKETING', {
            currentState: { adSpend: 10000, roas: 2.1 },
            desiredState: { adSpend: 10000, roas: 4.0, channels: ['LinkedIn', 'Google'] },
            changedBy: 'USER'
        });
        const marketingDomain = (await GrowthTwinService_1.GrowthTwinService.getDomains(businessId)).find(d => d.domain === 'MARKETING');
        if (!marketingDomain?.gap)
            throw new Error('Gap not computed');
        const gap = JSON.parse(marketingDomain.gap);
        if (!Array.isArray(gap) || gap.length === 0)
            throw new Error('Gap array empty');
        ...
        const history = await GrowthTwinService_1.GrowthTwinService.getHistory(businessId, 'MARKETING');
        if (history.length === 0)
        ...
        if (!context || !context.id)
            throw new Error('AI Operating Context not created');
        ...
        if (sectors.length < 5)
            throw new Error(`Expected 5+ sectors, got ${sectors.length}`);
        const saas = sectors_1.SectorManager.getSector('saas');
        if (!saas || saas.slug !== 'saas')
            throw new Error('SaaS sector not found');
        if (!saas.futureAIPromptTemplates['strategy-engine'])
            throw new Error('SaaS has no strategy engine prompt template');
        if (saas.marketingChannels.length === 0)
            throw new Error('SaaS has no marketing channels');
        ...
        if (kpis.length < 15)
            throw new Error(`Expected 15 KPIs, got ${kpis.length}`);
        const cac = kpis_1.KPIRegistry.getBySlug('cac');
        if (!cac)
            throw new Error('CAC KPI not found');
        if (!cac.healthyRange || !cac.warningRange || !cac.criticalRange)
            throw new Error('CAC ranges missing');
        const health = kpis_1.KPIRegistry.getHealthStatus('cac', 200);
        if (health !== 'HEALTHY')
            throw new Error(`Expected HEALTHY, got ${health}`);
        const critical = kpis_1.KPIRegistry.getHealthStatus('cac', 1000);
        if (critical !== 'CRITICAL')
            throw new Error(`Expected CRITICAL, got ${critical}`);
        ...
        if (contracts_1.ALL_ENGINE_CONTRACTS.length !== 6)
            throw new Error(`Expected 6 contracts, got ${contracts_1.ALL_ENGINE_CONTRACTS.length}`);
        const strategyContract = (0, contracts_1.getEngineContract)('strategy-engine');
        if (!strategyContract)
            throw new Error('Strategy Engine contract not found');
        if (strategyContract.futureDebateParticipants.length === 0)
            throw new Error('No debate participants defined');
        if (!strategyContract.confidenceRequirements.minimum)
            throw new Error('No confidence requirement');
        for (const contract of contracts_1.ALL_ENGINE_CONTRACTS) {
            if (!contract.failureConditions.length)
                throw new Error(`${contract.id} has no failure conditions`);
        }
        console.log(`  ✔ ${contracts_1.ALL_ENGINE_CONTRACTS.length} engine contracts loaded`);
        ...
        const readinessReports = await AIReadinessService_1.AIReadinessService.calculateAllReadiness(businessId);
        if (readinessReports.length !== 6)
            throw new Error(`Expected 6 reports, got ${readinessReports.length}`);
        for (const report of readinessReports) {
            ...
            if (!Array.isArray(report.recommendations))
                throw new Error(`No recommendations for ${report.engineId}`);
        }
        ...
        if (explained.length === 0)
            throw new Error('No explained questions returned');
        const firstQ = explained[0];
        if (!firstQ.whyItMatters)
            throw new Error('whyItMatters missing');
        if (!firstQ.whichEngineNeedsIt)
            throw new Error('whichEngineNeedsIt missing');
        if (firstQ.expectedConfidenceImprovement < 0)
            throw new Error('Invalid confidence improvement');
        ...
        const allQuestions = QuestionLibrary_1.QUESTION_LIBRARY;
        if (allQuestions.length < 10)
            throw new Error(`Question library too small: ${allQuestions.length}`);
        ...
        if (typeof state.overallCoverage !== 'number')
            throw new Error('overallCoverage missing');
        if (!Array.isArray(state.activeQuestions))
            throw new Error('activeQuestions missing');
