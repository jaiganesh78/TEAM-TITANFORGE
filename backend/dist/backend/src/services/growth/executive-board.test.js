...
const ExecutiveBoardService_1 = require("./ExecutiveBoardService");
...
async function createTestEnvironment() {
    const org = await prisma_1.prisma.organization.create({ data: { name: 'Executive Board Test Org' } });
    ...
    const analyticsSession = await prisma_1.prisma.analyticsSession.create({
        data: {
            businessId: TEST_BIZ_ID,
            status: 'COMPLETED'
        }
    });
    await prisma_1.prisma.businessEvolutionSnapshot.create({
        data: {
    ...
    const csSession = await prisma_1.prisma.customerSuccessSession.create({
        data: {
            businessId: TEST_BIZ_ID,
            status: 'COMPLETED'
        }
    });
    const csTwin = await prisma_1.prisma.customerDigitalTwin.create({
        data: {
...
async function cleanup(bizId) {
    ...
    const org = await prisma_1.prisma.business.findUnique({ where: { id: bizId } });
    if (org) {
        await prisma_1.prisma.business.delete({ where: { id: bizId } });
        await prisma_1.prisma.organization.delete({ where: { id: org.organizationId } });
    }
}
async function runExecutiveBoardTests() {
    ...
    try {
        ...
        assert(ExecutiveRepresentative_1.EXECUTIVE_REPRESENTATIVES.length === 6, 'Six engine representatives registered');
        ...
        const dbBrief = await prisma_1.prisma.executiveBrief.findFirst({ where: { businessId: bizId } });
        ...
        const dbRecs = await prisma_1.prisma.executiveRecommendation.findMany({ where: { businessId: bizId } });
        assert(dbRecs.length > 0, 'ExecutiveRecommendation entries persisted in database');
        const dbConflicts = await prisma_1.prisma.executiveConflict.findMany({ where: { businessId: bizId } });
        assert(dbConflicts.length > 0, 'ExecutiveConflict entries persisted in database');
        const dbOperatingPlan = await prisma_1.prisma.executiveOperatingPlan.findFirst({ where: { businessId: bizId } });
        assert(dbOperatingPlan !== null, 'ExecutiveOperatingPlan persisted in database');
        const dbRoadmap = await prisma_1.prisma.executiveRoadmap.findMany({ where: { businessId: bizId } });
        assert(dbRoadmap.length > 0, 'ExecutiveRoadmap entries persisted in database');
        const dbMeeting = await prisma_1.prisma.executiveMeeting.findFirst({ where: { businessId: bizId } });
        ...
        const dbDecision = await prisma_1.prisma.executiveDecision.findFirst({ where: { businessId: bizId } });
