...
exports.DiscoveryChatService = void 0;
const prisma_1 = require("../../database/prisma");
const DiscoveryEngine_1 = require("./DiscoveryEngine");
const DiscoveryPipeline_1 = require("./DiscoveryPipeline");
...
class DiscoveryChatService {
     ...
     */
    static async getOrCreateSession(businessId) {
        let session = await prisma_1.prisma.discoveryChatSession.findUnique({
            where: { businessId },
            include: { messages: { orderBy: { createdAt: 'asc' } } }
        });
        if (!session) {
            // 1. Evaluate current state using existing telemetry engine
            const telemetry = await DiscoveryEngine_1.DiscoveryEngine.evaluateState(businessId);
            const activePhase = DiscoveryPipeline_1.DiscoveryPipeline.getActivePhase(telemetry.categoryCoverage);
            // 2. Create session
            session = await prisma_1.prisma.discoveryChatSession.create({
                data: {
                    businessId,
                    status: 'IN_PROGRESS',
            ...
            const initialMessage = await prisma_1.prisma.discoveryMessage.create({
                data: {
     ...
     */
    static async respond(businessId, content) {
        ...
        if (!lastAssistantMsg || !lastAssistantMsg.questionId) {
            throw new errorMiddleware_1.AppError('No active question to respond to.', 400, 'NO_ACTIVE_QUESTION');
        }
        const question = QuestionLibrary_1.QUESTION_LIBRARY.find((q) => q.id === lastAssistantMsg.questionId);
        if (!question) {
            throw new errorMiddleware_1.AppError(`Active question not found in library: ${lastAssistantMsg.questionId}`, 404, 'NOT_FOUND');
        }
        // 1. Save user reply as a normalized message
        const userMsg = await prisma_1.prisma.discoveryMessage.create({
            data: {
                sessionId: session.id,
                role: 'user',
                content,
        ...
        let extractedValue = content.trim();
        ...
        await DiscoveryEngine_1.DiscoveryEngine.saveAnswer(businessId, question.id, extractedValue, 'KNOWN', 'USER');
        // Trigger Twin Update domain event to loose couple other modules
        await EventBroker_1.eventBroker.publish('TwinUpdated', { businessId, fieldPath: question.dbPath, value: extractedValue });
        // 3. Re-evaluate state telemetry
        const telemetry = await DiscoveryEngine_1.DiscoveryEngine.evaluateState(businessId);
        const activePhase = DiscoveryPipeline_1.DiscoveryPipeline.getActivePhase(telemetry.categoryCoverage);
        const phaseProgress = DiscoveryPipeline_1.DiscoveryPipeline.getPhaseProgress(activePhase, telemetry.categoryCoverage);
        ...
        const updatedSession = await prisma_1.prisma.discoveryChatSession.update({
            where: { id: session.id },
            data: {
                currentPhase: activePhase.key,
        ...
        const assistantMsg = await prisma_1.prisma.discoveryMessage.create({
            data: {
                sessionId: session.id,
                role: 'assistant',
                content: nextContent,
        ...
        await EventBroker_1.eventBroker.publish('ConfidenceChanged', { businessId, overallConfidence });
        return {
     ...
     */
    static selectNextQuestion(phaseKey, activeQuestions, answeredIds) {
        const phase = DiscoveryPipeline_1.DISCOVERY_PHASES.find((p) => p.key === phaseKey);
        if (!phase || phaseKey === 'VALIDATION')
            return null;
        // Filter questions that map to active phase categories and are not yet answered
        const phaseQuestions = activeQuestions.filter((q) => phase.categoryKeys.includes(q.category) && !answeredIds.includes(q.id));
        if (phaseQuestions.length === 0)
            return null;
        // Sort by confidenceImpact desc, then by discoveryPriority desc to maximize value
        const sorted = phaseQuestions.sort((a, b) => {
...
exports.DiscoveryChatService = DiscoveryChatService;
