...
exports.DiscoveryPipeline = exports.DISCOVERY_PHASES = void 0;
exports.DISCOVERY_PHASES = [
    {
...
class DiscoveryPipeline {
     ...
     */
    static getActivePhase(overallCoverage) {
        for (const phase of exports.DISCOVERY_PHASES) {
            if (phase.key === 'VALIDATION') {
                return phase;
            }
            // Check if all categories in this phase are complete (100% coverage)
            const allComplete = phase.categoryKeys.every((catKey) => (overallCoverage[catKey] ?? 0) >= 100);
            if (!allComplete) {
                return phase;
            }
        }
        return exports.DISCOVERY_PHASES[exports.DISCOVERY_PHASES.length - 1]; // Return VALIDATION by default
    }
    /**
     * Calculates the overall completion percent of the active phase.
     */
    static getPhaseProgress(phase, overallCoverage) {
        if (phase.key === 'VALIDATION')
            return 100;
        if (phase.categoryKeys.length === 0)
            return 0;
