...
exports.KPIRegistry = exports.KPI_REGISTRY = void 0;
...
class KPIRegistry {
    static getAll() {
        return exports.KPI_REGISTRY;
    }
    static getBySlug(slug) {
        return exports.KPI_REGISTRY.find(k => k.slug === slug);
    }
    static getByEngine(engineId) {
        return exports.KPI_REGISTRY.filter(k => k.owningEngine === engineId);
    }
    static getByGrowthDomain(domain) {
        return exports.KPI_REGISTRY.filter(k => k.growthDomain === domain);
    }
    static getBySlugs(slugs) {
        return exports.KPI_REGISTRY.filter(k => slugs.includes(k.slug));
    }
    static getBenchmark(slug, sectorSlug) {
        const kpi = this.getBySlug(slug);
        if (!kpi)
            return 0;
        const benchmarks = kpi.industryBenchmark;
        return benchmarks[sectorSlug] ?? benchmarks.default;
    }
    static getHealthStatus(slug, value) {
        const kpi = this.getBySlug(slug);
        if (!kpi)
            return 'UNKNOWN';
...
exports.KPIRegistry = KPIRegistry;
