...
export class RetrievalService {
  ...
  static async retrieve(params: {
    ...
    const queryVec = EmbeddingQueueService.generateEmbedding(params.query);
    ...
    const mergedMap: Map<string, { chunk: any; vectorScore: number; keywordScore: number }> = new Map();
    ...
    for (const vm of vectorMatches) {
      ...
      if (params.sourceType && vm.metadata.sourceType !== params.sourceType) continue;
