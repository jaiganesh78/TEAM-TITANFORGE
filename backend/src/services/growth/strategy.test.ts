...
import { ResponseParser, SwotSchema, RecommendationSchema } from '../../../../ai/providers/ResponseParser';
import { createMasterStrategyGraph } from './StrategyWorkflow';
import { StrategyEngineService } from './StrategyEngineService';
...
async function createTestEnvironment(): Promise<string> {
  const org = await prisma.organization.create({ data: { name: 'Strategy Test Org' } });
...
async function runStrategyTests() {
  ...
  try {
    ...
    `;
    const parsedSwot = ResponseParser.parseAndValidate(mockSwotJson, SwotSchema);
    ...
    const ctxPackage = await BusinessContextService.assembleContext(bizId, 'Context mapping evaluation');
    ...
    const nodeLogs = await prisma.graphNodeOutput.findMany({ where: { sessionId: session.id } });
    assert(nodeLogs.length > 0, `Recorded ${nodeLogs.length} GraphNodeOutput traces`);
    const runLogs = await prisma.graphExecutionLog.findMany({ where: { sessionId: session.id } });
    ...
    const fullSession = await StrategyEngineService.generateStrategy(bizId);
    ...
    const aiCtx = await prisma.aIOperatingContext.findUnique({ where: { businessId: bizId } });
    ...
    const memorySummary = await StrategyMemoryService.getStrategyMemorySummary(bizId);
    ...
    const dbRecs = await prisma.strategyRecommendation.findMany({ where: { businessId: bizId } });
    assert(dbRecs.length > 0, 'Fetched generated recommendations');
    const targetRec = dbRecs[0];

    const approved = await StrategyEngineService.submitFeedback(
      bizId,
      targetRec.id,
      'ACCEPT',
      'This mid-market strategy looks great, let us launch next week'
    );
    assert(approved.status === 'APPROVED', 'Recommendation marked APPROVED');

    // Verify Execution Plan generated
    const plan = await prisma.strategyExecutionPlan.findFirst({ where: { recommendationId: targetRec.id } });
    ...
    const readiness = await AIReadinessService.calculateReadiness(bizId, 'strategy-engine');
