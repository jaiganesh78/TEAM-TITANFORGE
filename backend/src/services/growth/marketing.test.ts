...
import { ResponseParser, CustomerJourneySchema, FunnelAnalysisSchema, MarketingChannelStrategySchema } from '../../../../ai/providers/ResponseParser';
import { createMasterMarketingGraph } from './MarketingWorkflow';
import { MarketingEngineService } from './MarketingEngineService';
import { MarketingMemoryService } from './MarketingMemoryService';
...
async function createTestEnvironment(): Promise<string> {
  const org = await prisma.organization.create({ data: { name: 'Marketing Test Org' } });
  ...
  const strategySession = await prisma.strategySession.create({
    data: {
...
async function cleanup(bizId: string) {
  const biz = await prisma.business.findUnique({ where: { id: bizId }, include: { organization: true } });
  if (biz) {
...
async function runMarketingTests() {
  ...
  try {
    ...
    const context = await BusinessContextService.assembleContext(bizId, 'Marketing Plan Assembly');
    ...
    `;
    const parsedJourney = ResponseParser.parseAndValidate(mockJourneyJson, CustomerJourneySchema);
    ...
    const graph = createMasterMarketingGraph();
    const session = await prisma.marketingSession.create({
      data: {
        businessId: bizId,
        status: 'IN_PROGRESS',
        contextVersion: 1
    ...
    const fullSession = await MarketingEngineService.generateMarketingPlan(bizId);
    ...
    const memSummary = await MarketingMemoryService.getCampaignHistorySummary(bizId);
    ...
    const camps = await prisma.marketingCampaign.findMany({ where: { businessId: bizId } });
    assert(camps.length > 0, 'Campaign records found');
    const targetCamp = camps[0];

    const approved = await MarketingEngineService.submitFeedback(
      bizId,
      targetCamp.id,
      'ACCEPT',
      'Looks perfect, launch the LinkedIn budget ad campaign'
    );
    assert(!!approved, 'Review feedback recorded');

    // Verify Execution Plan generated
    const execPlan = await prisma.marketingExecutionPlan.findFirst({ where: { campaignId: targetCamp.id } });
    ...
    const readiness = await AIReadinessService.calculateReadiness(bizId, 'marketing-engine');
