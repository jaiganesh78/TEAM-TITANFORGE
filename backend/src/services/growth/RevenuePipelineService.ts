...
export class RevenuePipelineService {
   ...
   */
  static async initializePipeline(businessId: string, pipelineName: string = 'Default Revenue Pipeline'): Promise<any> {
    // Check if pipeline already exists
    const existing = await prisma.revenuePipeline.findFirst({
      where: { businessId }
    });
    if (existing) return existing;

    const pipeline = await prisma.revenuePipeline.create({
      data: {
        businessId,
        name: pipelineName
      }
    ...
    for (const stage of standardStages) {
      await prisma.revenuePipelineStage.create({
        data: {
  ...
  static async transitionLead(
    ...
    const lead = await prisma.leadProfile.findUnique({
      where: { id: leadId },
      include: { stage: true }
    });

    if (!lead) throw new Error(`Lead profile not found: ${leadId}`);

    const targetStage = await prisma.revenuePipelineStage.findFirst({
      where: { businessId, stageName: targetStageName }
    });

    if (!targetStage) throw new Error(`Stage not found: ${targetStageName} for business ${businessId}`);

    // Update Lead Profile stage reference
    const updatedLead = await prisma.leadProfile.update({
      where: { id: leadId },
      data: {
    ...
    const lastHistory = await prisma.revenuePipelineHistory.findFirst({
    ...
    await prisma.revenuePipelineHistory.create({
      data: {
   ...
   */
  static async getPipelineHealth(businessId: string): Promise<any> {
    ...
    if (!pipeline) return { healthScore: 0, stages: [] };
    ...
    const stuckOpportunities: any[] = [];
    const now = new Date();

    for (const stage of pipeline.stages) {
      if (stage.averageTimeDays === 0) continue;
      const limitDays = stage.averageTimeDays * 1.5;

      for (const lead of stage.leads) {
        // Find how long they have been in this stage
        const history = await prisma.revenuePipelineHistory.findFirst({
