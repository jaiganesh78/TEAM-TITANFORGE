...
async function runConfigTests() {
  ...
  const ecom = SectorManager.getSector('ecommerce');
  assert(ecom.slug === 'ecommerce', 'eCommerce sector found');
  const ps = SectorManager.getSector('professional-services');
  assert(ps.slug === 'professional-services', 'Professional Services sector found');
  const retail = SectorManager.getSector('retail');
  assert(retail.slug === 'retail', 'Retail sector found');
  const mkt = SectorManager.getSector('marketplace');
  assert(mkt.slug === 'marketplace', 'Marketplace sector found');

  const generic = SectorManager.getSector('unknown-sector');
  assert(generic.slug === 'generic', 'Unknown sector falls back to generic');

  const saasKpis = SectorManager.getKpisForSector('saas');
  assert(saasKpis.length > 0, `SaaS KPIs resolved: ${saasKpis.length}`);

  const prompt = SectorManager.getPromptTemplate('saas', 'strategy-engine');
  assert(prompt.length > 20, 'SaaS strategy-engine prompt template found');

  const saasQuestions = SectorManager.getQuestionsForSector('saas');
  ...
  assert(KPIRegistry.getHealthStatus('cac', 200) === 'HEALTHY', 'CAC health: 200 → HEALTHY');
  assert(KPIRegistry.getHealthStatus('cac', 500) === 'WARNING', 'CAC health: 500 → WARNING');
  assert(KPIRegistry.getHealthStatus('cac', 1000) === 'CRITICAL', 'CAC health: 1000 → CRITICAL');
  assert(KPIRegistry.getBenchmark('cac', 'saas') > 0, 'CAC SaaS benchmark > 0');

  const ltv = KPIRegistry.getBySlug('ltv');
  assert(!!ltv, 'LTV KPI found');
  const marketingKpis = KPIRegistry.getByEngine('marketing-engine');
  assert(marketingKpis.length > 0, `${marketingKpis.length} Marketing Engine KPIs`);
  const analyticsKpis = KPIRegistry.getByEngine('analytics-engine');
  ...
  // ─────────────────────────────────────────────
  console.log('\nTest 3: Engine Contracts');
  assert(ALL_ENGINE_CONTRACTS.length === 6, '6 engine contracts defined');
  ...
  for (const id of engines) {
    const contract = getEngineContract(id);
  ...
  // ─────────────────────────────────────────────
  console.log('\nTest 4: Question Library — Backward Compatibility');
  assert(QUESTION_LIBRARY.length >= 10, `${QUESTION_LIBRARY.length} questions in library`);
  const q0 = QUESTION_LIBRARY[0];
  ...
  const withGrowthDomain = QUESTION_LIBRARY.filter(q => q.growthDomain);
  assert(withGrowthDomain.length > 0, `${withGrowthDomain.length} questions have growthDomain`);
  const withEngine = QUESTION_LIBRARY.filter(q => q.relatedEngine);
  ...
  const answered = QUESTION_LIBRARY.slice(0, 5).map(q => q.id);
  const withAnswered = DiscoveryExplainer.computeNextQuestions({
    businessId: 'test-id',
    sectorSlug: 'saas',
    businessStage: 'GROWTH',
