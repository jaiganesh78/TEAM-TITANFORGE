...
export class LeadEngineService {
   ...
   */
  static async runLeadEngine(businessId: string): Promise<any> {
    // 1. Validate readiness before running
    const readiness = await AIReadinessService.calculateReadiness(businessId, 'lead-generation-engine');
    ...
    const contextPackage = await BusinessContextService.assembleContext(businessId, 'Lead Generation and Ideal Customer Profiles (ICP)');
    ...
    try {
      ...
      for (const rec of recommendations) {
        const dbRec = await prisma.leadRecommendation.create({
          data: {
            sessionId: session.id,
            businessId,
            title: rec.title,
        ...
        for (const lead of leads) {
          ...
          const stage = await prisma.revenuePipelineStage.findFirst({
      ...
      if (forecast) {
        await prisma.leadForecast.create({
          data: {
            sessionId: session.id,
            businessId,
      ...
      for (const p of playbooks) {
        await prisma.leadPlaybook.create({
          data: {
            sessionId: session.id,
            businessId,
            name: p.name,
  ...
  static async handleFeedback(
    ...
    if (!session) throw new Error(`Lead session not found: ${sessionId}`);

    // Fetch primary recommendation
    const rec = session.recommendations[0];
    if (!rec) throw new Error(`No lead recommendation in session ${sessionId}`);

    // Create Feedback log
    const feedback = await prisma.leadFeedback.create({
      data: {
        sessionId,
    ...
    if (action === 'ACCEPT') {
      ...
      await LeadMemoryService.recordApprovedStrategy(businessId, rec.title);

      // Create an execution plan checklist
      await prisma.leadExecutionPlan.create({
        data: {
          sessionId,
          recommendationId: rec.id,
          businessId,
      ...
      if (!context) {
        await prisma.aIOperatingContext.create({
          data: {
            businessId,
      ...
      await LeadMemoryService.recordRejectedStrategy(businessId, rec.title, feedbackText);
