...
import { ResponseParser, SnapshotComparisonSchema } from '../../../../ai/providers/ResponseParser';

export class BusinessEvolutionService {
  ...
  static async captureSnapshot(
    ...
    // Fetch KPI states
    const kpis = await prisma.businessKPI.findMany({ where: { businessId } });
    ...
    const pipeline = await prisma.revenuePipeline.findFirst({
      where: { businessId },
      include: { stages: true }
    });
    ...
    const twin = await prisma.growthTwinSummary.findFirst({ where: { businessId } });
    const twinConfidence = twin ? twin.overallConfidence : 80.0;

    // Fetch Knowledge Statistics
    const knowledgeStats = await prisma.knowledgeStatistics.findUnique({ where: { businessId } });
    ...
    const strategyRecs = await prisma.strategyRecommendation.findMany({ where: { businessId } });
    ...
    const leadMemory = await prisma.leadMemory.findFirst({ where: { businessId } });
    const salesMemory = await prisma.salesMemory.findFirst({ where: { businessId } });
    const analyticsMemory = await prisma.analyticsMemory.findFirst({ where: { businessId } });
    ...
    return prisma.businessEvolutionSnapshot.create({
      data: {
        sessionId,
        businessId,
        version: nextVersion,
  ...
  static async compareSnapshots(
    ...
    const source = await prisma.businessEvolutionSnapshot.findUnique({ where: { id: sourceSnapshotId } });
    const target = await prisma.businessEvolutionSnapshot.findUnique({ where: { id: targetSnapshotId } });
...
`;
    ...
    const parsed = ResponseParser.parseAndValidate(res.text, SnapshotComparisonSchema);

    // Save comparison to database
    return prisma.snapshotComparison.create({
      data: {
        sessionId,
        businessId,
        sourceSnapshotId,
   ...
   */
  static async recordRecommendationImpact(
    businessId: string,
    ...
    return prisma.aIImpactAnalysis.create({
      data: {
        businessId,
        recommendationId,
        title,
  ...
  static async logDecision(
    ...
    return prisma.decisionIntelligence.create({
      data: {
        businessId,
        decision,
        reason,
   ...
   */
  static async recordDecisionImpact(decisionId: string, actualImpact: number) {
    return prisma.decisionIntelligence.update({
      where: { id: decisionId },
      data: { actualImpact }
    });
  }
   ...
   */
  static async calculateForecastAccuracy(businessId: string) {
    const historicalSnapshots = await prisma.forecastHistory.findMany({
      where: { businessId }
    });
   ...
   */
  static async generateQuarterlyStory(businessId: string): Promise<string> {
    const snapshots = await prisma.businessEvolutionSnapshot.findMany({
      where: { businessId },
      orderBy: { version: 'asc' }
    });

    const decisions = await prisma.decisionIntelligence.findMany({
      where: { businessId },
