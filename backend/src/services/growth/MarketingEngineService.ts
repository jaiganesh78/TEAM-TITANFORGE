...
export class MarketingEngineService {
   ...
   */
  static async generateMarketingPlan(businessId: string): Promise<any> {
    // 1. Validate readiness before running
    const readiness = await AIReadinessService.calculateReadiness(businessId, 'marketing-engine');
    ...
    const contextPackage = await BusinessContextService.assembleContext(businessId, 'Company Marketing Plan');

    // 3. Create context snapshot record
    const snapshot = await BusinessContextService.createSnapshot(
      businessId,
      'Marketing Session Target',
    ...
    try {
      ...
      const updatedSession = await prisma.marketingSession.update({
        where: { id: session.id },
        data: {
          status: 'COMPLETED',
      ...
      for (const camp of campaignsList) {
        ...
        for (const fact of factsUsed) {
          await prisma.campaignEvidence.create({
            data: {
              sessionId: session.id,
      ...
      for (const item of calendarItems) {
        await prisma.marketingCalendar.create({
          data: {
            sessionId: session.id,
            businessId,
   ...
   */
  private static async syncToAIOperatingContext(businessId: string, finalState: any): Promise<void> {
    ...
    const newExperiments = finalState.experimentPortfolio?.experiments?.map((e: any) => e.hypothesis) || [];
  ...
  static async submitFeedback(
    ...
    await prisma.marketingFeedback.create({
      data: {
    ...
    if (action === 'ACCEPT') {
      await prisma.marketingExecutionPlan.create({
        data: {
          sessionId: campaign.sessionId,
          campaignId,
          businessId,
