...
import { BusinessEvolutionService } from '../services/growth/BusinessEvolutionService';
 ...
 */
router.post('/run/:id', asyncHandler(async (req: Request, res: Response) => {
  const businessId = req.params.id;
  const sessionId = await AnalyticsEngineService.runAnalyticsEngine(businessId);
 ...
 */
router.post('/feedback/:id', asyncHandler(async (req: Request, res: Response) => {
  const sessionId = req.params.id;
  ...
  const feedback = await AnalyticsEngineService.handleFeedback(businessId, sessionId, action, feedbackText);
 ...
 */
router.get('/evolution/:id', asyncHandler(async (req: Request, res: Response) => {
  const businessId = req.params.id;
  const timeline = await prisma.businessEvolutionSnapshot.findMany({
    where: { businessId },
...
router.post('/compare', asyncHandler(async (req: Request, res: Response) => {
  ...
  const comparison = await BusinessEvolutionService.compareSnapshots(
    businessId,
    sessionId,
    sourceSnapshotId,
    targetSnapshotId
 ...
 */
router.get('/decisions/:id', asyncHandler(async (req: Request, res: Response) => {
  const businessId = req.params.id;
 ...
 */
router.get('/story/:id', asyncHandler(async (req: Request, res: Response) => {
  const businessId = req.params.id;
  const storyText = await BusinessEvolutionService.generateQuarterlyStory(businessId);
