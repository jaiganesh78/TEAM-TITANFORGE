import {
  IDiscoveryEngine,
  IBusinessClassificationEngine,
  ICoverageEngine,
  IQuestionEngine,
  IWebsiteAnalysisEngine,
  IDocumentAnalysisEngine,
  IRecommendationEngine,
  IReflectionEngine,
  IHistoricalMemoryEngine,
  IDecisionSimulatorEngine,
...
export class MockDiscoveryEngine implements IDiscoveryEngine {
  async initializeFlow(organizationId: string): Promise<DiscoveryFlowState> {
    return {
      organizationId,
      industry: 'TBD',
      completedCategories: [],
  ...
  async detectIndustry(description: string): Promise<string> {
  ...
  async getNextQuestions(state: DiscoveryFlowState): Promise<Question[]> {
  ...
  async submitAnswers(organizationId: string, answers: Record<string, any>): Promise<DiscoveryFlowState> {
    return {
      organizationId,
  ...
  async enrichFromWebsite(organizationId: string, url: string): Promise<Partial<BusinessDigitalTwin>> {
    return {
      identity: {
        id: 'twin-identity-1',
  ...
  async enrichFromDocument(organizationId: string, documentId: string): Promise<Partial<BusinessDigitalTwin>> {
    return {
      finance: {
        cashOnHand: 550000,
        burnRate: 45000,
...
export class MockBusinessClassificationEngine implements IBusinessClassificationEngine {
  async classifyBusiness(twin: BusinessDigitalTwin): Promise<{ primarySector: string; secondarySector: string; businessModel: string[] }> {
    return {
...
export class MockCoverageEngine implements ICoverageEngine {
  async calculateCoverage(twin: BusinessDigitalTwin): Promise<{ overallCoverage: number; missingFields: string[]; categoryBreakdown: Record<string, number> }> {
    return {
      overallCoverage: 65,
      missingFields: [
...
export class MockQuestionEngine implements IQuestionEngine {
  async generateDynamicQuestions(twin: BusinessDigitalTwin, count: number = 3): Promise<Question[]> {
    return [
      {
        id: 'dyn-q1',
...
export class MockWebsiteAnalysisEngine implements IWebsiteAnalysisEngine {
...
export class MockDocumentAnalysisEngine implements IDocumentAnalysisEngine {
  async parseDocument(filePath: string, fileType: string): Promise<{
...
export class MockRecommendationEngine implements IRecommendationEngine {
  async generateRecommendations(twin: BusinessDigitalTwin): Promise<Recommendation[]> {
    return [
      {
        id: 'rec-1',
  ...
  async getRecommendationById(id: string): Promise<Recommendation | null> {
    const list = await this.generateRecommendations({} as any);
    return list.find(r => r.id === id) || null;
  }
}

export class MockReflectionEngine implements IReflectionEngine {
  async reflectOnRecommendation(rec: Recommendation): Promise<RecommendationReflection> {
    return {
  ...
  async evaluateDebate(rounds: DebateRound[]): Promise<{ consensusReached: boolean; recommendations: Recommendation[] }> {
    const engine = new MockRecommendationEngine();
    const recs = await engine.generateRecommendations({} as any);
    return {
...
export class MockHistoricalMemoryEngine implements IHistoricalMemoryEngine {
  async logDecisionOutcome(decisionId: string, observedKPIs: Record<string, number>): Promise<void> {
    console.log(`Log decision outcome: ${decisionId}`, observedKPIs);
  }

  async querySimilarDecisions(context: string): Promise<any[]> {
    return [
      {
...
export class MockDecisionSimulatorEngine implements IDecisionSimulatorEngine {
...
export class MockExplainabilityEngine implements IExplainabilityEngine {
  async generateExplanationTrace(recId: string): Promise<ExplanationTrace> {
    return {
...
export class MockTimelineEngine implements ITimelineEngine {
  async generateTimeline(organizationId: string): Promise<{ events: any[] }> {
    return {
      events: [
        {
          id: 't-ev-1',
